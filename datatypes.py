class User(object):
  ...
  def __repr__(self):
    return '{0} ({1}): isAdmin: {2}, isMCRMember: {3}, isAssociateMember: {4}, isCRA: {5}, isCollegeBill: {6}'.format(self.displayName(), self.userID, self.isAdmin, self.isMCRMember, self.isAssociateMember, self.isCRA, self.isCollegeBill)
...
class Event(object):
  def __init__(self, eventID, name, totalTickets, allocatedTickets, maxGuests, openToAssociateMembers, openToMCRMembers, openToCRAs, openToNonClareAssociateMembers, costPrimary, costGuest, eventDate, bookingOpenDate, bookingCloseDate, sent):
  ...
  def __repr__(self):
    return '{0}: {1}/{2}. openToMCRMembers: {3}, openToAssociateMembers: {4}, openToCRAs: {5}, openToNonClareAssociateMembers: {6}. {7}'.format(self.name, self.allocatedTickets, self.totalTickets, self.openToMCRMembers, self.openToAssociateMembers, self.openToCRAs, self.openToNonClareAssociateMembers, self.eventDate)
...
class EventDefaults(object):
  ...
  def __repr__(self):
    return 'costPrimary: {0}, costGuest: {1}, maxGuests: {2}, totalTickets: {3}'.format(self.costPrimary, self.costGuest, self.maxGuests, self.totalTickets)
