...
def ravenUsers(crsid=None):
  cur = getMySQLCursor()
  if crsid is None:
    cur.execute('SELECT crsid, e_adm, mcr_member, associate_member, cra, college_bill FROM access WHERE e_view = 1 AND e_book = 1 AND enabled = 1')
  else:
    cur.execute('SELECT crsid, e_adm, mcr_member, associate_member, cra, college_bill FROM access WHERE e_view = 1 AND e_book = 1 AND enabled = 1 AND crsid = "{0}"'.format(crsid))
  users = []
  for row in cur.fetchall():
...
def getEvents(eventID=None):
  cur = getMySQLCursor()
  if eventID is None:
    cur.execute('SELECT id, name, total_guests, current_guests, max_guests, associate_member, mcr_member, cra, non_clare_associate_member, cost_normal, cost_second, event_date, open_date, close_date, sent FROM {0} ORDER BY event_date ASC'.format(mcrevents_eventslist))
  else:
    cur.execute('SELECT id, name, total_guests, current_guests, max_guests, associate_member, mcr_member, cra, non_clare_associate_member, cost_normal, cost_second, event_date, open_date, close_date, sent FROM {0} WHERE id = %s ORDER BY event_date ASC'.format(mcrevents_eventslist), (eventID))
  events = []
  for row in cur.fetchall():
    from datatypes import Event
    event = Event(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], row[11], row[12], row[13], row[14])
    events.append(event)
  return events
...
def storeNewEvent(event):
  ...
  cur.execute('INSERT INTO {0} (name, total_guests, current_guests, max_guests, associate_member,\
      ...
      event.openToMCRMembers, event.openToCRAs, event.openToNonClareAssociateMembers, event.costPrimary, event.costGuest,\
...
def isUserBookedInEvent(eventID, userID, isAdminBooking=False):
  cur = getMySQLCursor()
  cur.execute('SELECT id FROM {0} WHERE eventid = %s AND booker = %s AND admin = %s'.format(mcrevents_booking), (eventID, userID, isAdminBooking))
...
def getBookings(eventID, userID=None, isAdminBooking=None):
  ...
  if userID is None:
    if isAdminBooking is None:
      cur.execute('SELECT id, booker, admin, tickets FROM {0} WHERE eventid = %s'.format(mcrevents_booking), (eventID))
    else:
      cur.execute('SELECT id, booker, admin, tickets FROM {0} WHERE eventid = %s AND admin = %s'.format(mcrevents_booking), (eventID, isAdminBooking))
  else:
    cur.execute('SELECT id, booker, admin, tickets FROM {0} WHERE eventid = %s AND booker = %s AND admin = %s'.format(mcrevents_booking), (eventID, userID, isAdminBooking))
  bookings = []
  for row in cur.fetchall():
    from datatypes import Booking, BookingDetails
    booking = Booking(row[0], row[1], row[2], row[3])
    cur.execute('SELECT type, id, name, diet, other FROM {0} WHERE eventid = %s AND bookingid = %s'.format(mcrevents_booking_details), (eventID, booking.bookingID))
    for row in cur.fetchall():
...
def _getQueueEntriesWorker(cur, eventID, userID=None):
  if userID is None:
    cur.execute('SELECT id, booker, admin, tickets FROM {0} WHERE eventid = %s ORDER BY id ASC'.format(mcrevents_queue), (eventID))
  else:
    cur.execute('SELECT id, booker, admin, tickets FROM {0} WHERE eventid = %s AND booker = %s ORDER BY id ASC'.format(mcrevents_queue), (eventID, userID))
  queueEntries = []
  for row in cur.fetchall():
    from datatypes import Booking, BookingDetails
    queueEntry = Booking(row[0], row[1], row[2], row[3])
    cur.execute('SELECT type, id, name, diet, other FROM {0} WHERE eventid = %s AND bookingid = %s'.format(mcrevents_queue_details), (eventID, queueEntry.bookingID))
    for row in cur.fetchall():
      details = BookingDetails(row[1], row[2], row[3], row[4])
      if row[0] == '1':
...
def makeBookingIfSpace(event, user, isAdminBooking, numTickets):
  ...
  for i in range(numTickets):
    ...
    cur.execute('INSERT INTO {0} (bookingid, eventid, booker, admin, type, name,\
        ...
        'None', ''))
  cur.execute('UPDATE {0} SET current_guests = %s WHERE id = %s'.format(mcrevents_eventslist), (current_guests+numTickets, event.eventID))
  db.commit()
  _unlockTables(cur)
  db.close()
...
def updateBookingDetails(booking):
  ...
  cur.execute('UPDATE {0} SET diet = %s, other = %s WHERE id = %s'.format(mcrevents_booking_details), (booking.primaryDetails.diet, booking.primaryDetails.other, booking.primaryDetails.detailsID))
  for guestDetails in booking.guestDetails:
    cur.execute('UPDATE {0} SET name = %s, diet = %s, other = %s WHERE id = %s'.format(mcrevents_booking_details), (guestDetails.name, guestDetails.diet, guestDetails.other, guestDetails.detailsID))
  db.commit()
  _unlockTables(cur)
  db.close()
...
def updateQueueDetails(booking):
  ...
  cur.execute('UPDATE {0} SET diet = %s, other = %s WHERE id = %s'.format(mcrevents_queue_details), (booking.primaryDetails.diet, booking.primaryDetails.other, booking.primaryDetails.detailsID))
  for guestDetails in booking.guestDetails:
    cur.execute('UPDATE {0} SET name = %s, diet = %s, other = %s WHERE id = %s'.format(mcrevents_queue_details), (guestDetails.name, guestDetails.diet, guestDetails.other, guestDetails.detailsID))
  db.commit()
  _unlockTables(cur)
  db.close()
...
def unbookGuest(eventID, bookingID, detailsID):
  ...
  cur.execute('UPDATE {0} SET current_guests = %s WHERE id = %s'.format(mcrevents_eventslist), (current_guests-1, eventID))
...
def deleteBookings(eventID, user, isAdminBooking=False):
  ...
  cur.execute('SELECT tickets FROM {0} WHERE eventid = %s AND booker = %s AND admin = %s'.format(mcrevents_booking), (eventID, user.userID, isAdminBooking))
  numTickets = cur.fetchone()[0]
  cur.execute('SELECT current_guests FROM {0} WHERE id = %s'.format(mcrevents_eventslist), (eventID))
  current_guests = cur.fetchone()[0]

  cur.execute('DELETE FROM {0} WHERE eventid = %s AND booker = %s AND admin = %s'.format(mcrevents_booking), (eventID, user.userID, isAdminBooking))
  cur.execute('DELETE FROM {0} WHERE eventid = %s AND booker = %s AND admin = %s'.format(mcrevents_booking_details), (eventID, user.userID, isAdminBooking))
  cur.execute('UPDATE {0} SET current_guests = %s WHERE id = %s'.format(mcrevents_eventslist), (current_guests-numTickets, eventID))
  db.commit()
  shouldNotifyList = _fillEmptySpacesFromQueueWorker(cur, db, eventID)
  _unlockTables(cur)
  db.close()
...
Kind regards,
Clare MCR Committee'''.format(user.displayName(), event.name, event.eventDate.strftime('%A, %-d %B at %H:%M'),userID,event.costPrimary,event.costGuest)
  else:
    body = '''Dear {0},
...
def _insertBookingWorker(cur, db, current_guests, eventID, booking):
  ...
  cur.execute('UPDATE {0} SET current_guests = %s WHERE id = %s'.format(mcrevents_eventslist), (current_guests+booking.numTickets, eventID))
  db.commit()
