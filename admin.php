...
echo "<html>\n";
echo "<head>\n";
echo "<title> The Clare MCR Events Booking System Administrator Page</title>\n";
...
function display_presend() {
    ...
    $statement = $dbh->prepare("SELECT * FROM " . $my_pre . "eventslist WHERE close_date < NOW() AND sent='N' ORDER BY event_date");
    $statement->execute();
...
function display_open() {
    ...
    $statement = $dbh->prepare("SELECT * FROM " . $my_pre . "eventslist WHERE open_date < NOW() AND close_date > NOW() ORDER BY event_date");
    $statement->execute();
...
function reopen_booking($eventid) {
    ...
    $statement = $dbh->prepare("UPDATE " . $my_pre . "eventslist SET close_date=:closedate WHERE id=:eventid");
    $statement->bindValue(":closedate", $datetime);
    $statement->bindValue(":eventid", $eventid);
    $statement->execute();

    # As the guestlist may change, standby to resend.

    $statement = $dbh->prepare("UPDATE " . $my_pre . "eventslist SET sent='N' WHERE id=:eventid");
    $statement->bindValue(":eventid", $eventid);
    $statement->execute();
...
function close_booking($eventid) {
    ...
    $statement = $dbh->prepare("UPDATE " . $my_pre . "eventslist SET close_date=NOW(), sent='N' WHERE id=:eventid");
    $statement->bindValue(":eventid", $eventid);
    $statement->execute();
...
function display_outstanding() {
    ...
    $statement = $dbh->prepare("SELECT * FROM " . $my_pre . "eventslist WHERE close_date < NOW() AND event_date > NOW() AND sent='Y' ORDER BY event_date");
    $statement->execute();
    $result = $statement->fetchAll(PDO::FETCH_ASSOC);

    $num_events = count($result);
    ...
    if ($num_events == 0) {
	    ...
	    echo "<td><input type=\"submit\" name=\"reopenbooking\" value=\"Reopen Booking\"> ";
            echo "<select name=\"time\" value=\"\"></option>";
...
function mark_complete($eventid) {
    ...
    $statement = $dbh->prepare("UPDATE " . $my_pre . "eventslist SET sent='Y' WHERE id=:eventid");
    $statement->bindValue(":eventid", $eventid);
    $statement->execute();
...
function show_guestlist($eventid) {
    ...
    echo "<hr/>";
    echo "<form method=\"post\" action=\"" . $SERVER_['PHP_SELF'] . "\">";
...
function show_billing($eventid) {
    ...
    $statement = $dbh->prepare("SELECT cost_normal,cost_second,event_date FROM " . $my_pre . "eventslist WHERE id=:id");
    ...
    $statement = $dbh->prepare("SELECT booker, SUM(tickets) as tot_tickets FROM " . $my_pre . "booking WHERE eventid=:id AND admin=1 GROUP BY eventid");
    $statement->bindValue(":id", $eventid);
    $statement->execute();
    ...
    $statement = $dbh->prepare("SELECT booker, SUM(tickets) as tot_tickets FROM " . $my_pre . "booking WHERE eventid=:id AND admin=0 GROUP BY booker");
    $statement->bindValue(":id", $eventid);
    $statement->execute();
    ...
    foreach ($result as $booking) {
        ...
        if ($booker->getValue('name') == $booker->getValue('crsid')) {
            ...
            $statement = $dbh->prepare("SELECT name FROM " . $my_pre . "booking_details WHERE bookingid=:id AND booker=:booker AND eventid=:eventid AND type='1'");
            $statement->bindValue(":id", $booking['bookingid']);
            $statement->bindValue(":booker", $booking['booker']);
            $statement->bindValue(":eventid", $eventid);

            $statement->execute();

            $result_n = $statement->fetch(PDO::FETCH_ASSOC);
...
function send_billing($eventid) {
    ...
    $statement = $dbh->prepare("SELECT SUM(tickets) as tot_tickets FROM " . $my_pre . "booking WHERE eventid=:id AND admin=1 GROUP BY eventid");
    $statement->bindValue(":id", $eventid);
    $statement->execute();
    ...
    $statement = $dbh->prepare("SELECT id as bookingid, booker, SUM(tickets) as tot_tickets FROM " . $my_pre . "booking WHERE eventid=:id AND admin=0 GROUP BY booker");
    $statement->bindValue(":id", $eventid);
    $statement->execute();
...
function newEventForm() {
    ...
    $timenos = array('Midnight' => '00:00:00', '9AM' => '09:00:00', '10AM' => '10:00:00', '11AM' => '11:00:00', 'Midday' => '12:00:00', '1PM' => '13:00:00', '2PM' => '14:00:00', '3PM' => '15:00:00', '4PM' => '16:00:00', '5PM' => '17:00:00', '6PM' => '18:00:00', '7PM' => '19:00:00', '8PM' => '20:00:00', '9PM' => '21:00:00', '10PM' => '22:00:00', '11PM' => '23:00:00');
    ...
    $statement = $dbh->prepare("SELECT name,value FROM " . $my_pre . "defaults");
    $statement->execute();
...
function create_event() {
    ...
    echo "<p><b>Event Name: </b>" . $escaped_event_name . "</p>";
...
function deleteEventForm() {
    ...
    $statement = $dbh->prepare("SELECT * FROM " . $my_pre . "eventslist WHERE NOW() < event_date ORDER BY event_date");
    $statement->execute();
    $result = $statement->fetchAll(PDO::FETCH_ASSOC);
...
function delete_event() {
    ...
    $statement = $dbh->prepare("SELECT name,event_date FROM " . $my_pre . "eventslist WHERE id=:eventid");
    $statement->bindValue(":eventid", $_POST['delete_selection']);
    $statement->execute();
    $result = $statement->fetch(PDO::FETCH_ASSOC);
    ...
    $statement = $dbh->prepare("DELETE FROM " . $my_pre . "eventslist WHERE id=:eventid");
    $statement->bindValue(":eventid", $_POST['delete_selection']);
    $statement->execute();
    ...
    $statement = $dbh->prepare("SELECT DISTINCT booker FROM " . $my_pre . "booking WHERE eventid=:eventid");
    $statement->bindValue(":eventid", $_POST['delete_selection']);
    $statement->execute();
    ...
    foreach ($result as $booker) {
        ...
        $statement = $dbh->prepare("DELETE FROM " . $my_pre . "booking WHERE eventid=:eventid AND booker=:booker");
        $statement->bindValue(":eventid", $_POST['delete_selection']);
        $statement->bindValue(":booker", $crsid);
        $statement->execute();

        # And remove the tickets from the tickets list
        $statement = $dbh->prepare("DELETE FROM " . $my_pre . "booking_details WHERE eventid=:eventid AND booker=:booker");
        $statement->bindValue(":eventid", $_POST['delete_selection']);
        $statement->bindValue(":booker", $crsid);
        $statement->execute();
    }
    ...
    $statement = $dbh->prepare("SELECT DISTINCT booker FROM " . $my_pre . "queue WHERE eventid=:eventid");
    $statement->bindValue(":eventid", $_POST['delete_selection']);
    $statement->execute();
    ...
    foreach ($result as $booker) {
        ...
        $statement = $dbh->prepare("DELETE FROM " . $my_pre . "queue WHERE eventid=:eventid AND booker=:booker");
        $statement->bindValue(":eventid", $_POST['delete_selection']);
        $statement->bindValue(":booker", $crsid);
        $statement->execute();

        # And remove the tickets from the queued tickets list
        $statement = $dbh->prepare("DELETE FROM " . $my_pre . "queue_details WHERE eventid=:eventid AND booker=:booker");
        $statement->bindValue(":eventid", $_POST['delete_selection']);
        $statement->bindValue(":booker", $crsid);
        $statement->execute();
    }
...
function editDefaultForm() {
    ...
    $statement = $dbh->prepare("SELECT * FROM " . $my_pre . "defaults");
    $statement->execute();
    ...
    foreach ($result as $variablearray) {
        ...
        echo "<input type=\"hidden\" name=\"variablearray[".$i."][id]\" value=\"" . $variablearray['id'] . "\"></input>";
        echo "<td><input type=\"text\" name=\"variablearray[".$i."][value]\" value=\"" . $variablearray['value'] . "\"></input></td>";
        echo "</tr>";
        $i++;
    }
...
function editDefault() {
    ...
    foreach ($_POST['variablearray'] as $var) {
        Validator::isNumeric($var['value'], $var['id']);
    }
    ...
    foreach ($_POST['variablearray'] as $var) {
        ...
        $statement = $dbh->prepare("UPDATE " . $my_pre . "defaults SET value=:value where id=:id");

        $statement->bindValue(":value", $var['value']);
        $statement->bindValue(":id", $var['id']);
...
function editEvent($eventid) {
    ...
    echo "<dl>";
    echo "<dt>Event name</dt>";
    echo "<dd><input type=\"text\" name=\"event_name\" required value=\"". $name ."\"placeholder=\"e.g. MCR Formal\" title=\"Please enter event name. The event date will automatically be added.\" size=\"30\" maxlength=\"50\"></dd>";
    echo "<dt>Total number of available tickets</dt>";
    echo "<dd><input type=\"number\" min=\"0\" size=\"5\" name=\"total_guests\" value=\"" . $def_total_guests . "\" required></dd>";
    echo "<dt>Maximum guests each person can bring</dt>";
    echo "<dd><input type=\"number\" min=\"0\" size=\"5\" name=\"max_guests\" value=\"" . $def_max_guests . "\" required></dd>";
    ...
    # Date picking for the event date
    echo "<dd>";
	echo '<table border="0" class="datetime">';
    echo "<tr><td><input type=\"date\" name=\"event[date]\" min=\"". date('Y-m-d') . "\"  value=\"". $event_date->format('Y-m-d') . "\" /></td>";
	echo "<td><input type=\"time\" name=\"event[time]\" min=\"00:00\"  value=\"". $event_date->format('H:i') . "\"  step=\"900\" required/></td></tr>";
	echo '</table>';
    echo "</dd></dl>";

    echo "<dt>Enter the opening date for the event</dt>";
    # Date picking for the event date
    echo "<dd>";
	echo '<table border="0" class="datetime">';
    echo "<tr><td><input type=\"date\" name=\"open[date]\" value=\"". $open_date->format('Y-m-d') . "\" /></td>";
	echo "<td><input type=\"time\" name=\"open[time]\" min=\"00:00\"  value=\"". $open_date->format('H:i') . "\"  step=\"900\" required/></td></tr>";
	echo '</table>';
    echo "</dd></dl>";

 	echo "<dt>Enter the closing date for the event</dt>";
    # Date picking for the event date
    echo "<dd>";
	echo '<table border="0" class="datetime">';
    echo "<tr><td><input type=\"date\" name=\"close[date]\" min=\"". date('Y-m-d') . "\"  value=\"". $close_date->format('Y-m-d') . "\" /></td>";
	echo "<td><input type=\"time\" name=\"close[time]\" min=\"00:00\"  value=\"". $close_date->format('H:i') . "\"  step=\"900\" required/></td></tr>";
	echo '</table>';
    echo "</dd></dl>";
